...
public boolean specialEleven(int n) {
    ...
    // Check if n is one more than a multiple of 11
    else if (n % 11 == 1) {
        return true;
    }
    ...
    // Check if n is 1 more than a multiple of 20
    if (n % 20 == 1) {
        return true;
    }
    // Check if n is 2 more than a multiple of 20
    else if (n % 20 == 2) {
        return true;
    }
    // If n is neither 1 nor 2 more than a multiple of 20
    else {
        return false;
    }
}
    ...
    // Check if n is 1 less than a multiple of 20
    if (n % 20 == 19) {
        return true;
    }
    // Check if n is 2 less than a multiple of 20
    else if (n % 20 == 18) {
        return true;
    }
    // If n is neither 1 nor 2 less than a multiple of 20
    else {
        return false;
    }
}
    ...
    // Check if either value is a teen (in the range 13..19)
    ...
    // Get the rightmost digit of a, b, and c using the % operator
    int lastA = a % 10; // Rightmost digit of a
    int lastB = b % 10; // Rightmost digit of b
    int lastC = c % 10; // Rightmost digit of c

    // Check if at least two of the rightmost digits are the same
    ...
    if (isMorning) {
        return isMom; // Answer if it's your mom calling
    }
    ...
    if (vacation) {
        // For vacation, weekends are off, weekdays are 10:00
        ...
        // Not on vacation, regular alarm times
        if (day == 0 || day == 6) { // Sunday or Saturday
            return "10:00";
        } else { // Weekdays
            return "7:00";
    ...
    // If either tea or candy is less than 5, the party is bad
    if (tea < 5 || candy < 5) {
        return 0;
    }
    
    // If either tea or candy is at least double the amount of the other, the party is great
    if (tea >= 2 * candy || candy >= 2 * tea) {
        return 2;
    }
    
    // If both tea and candy are at least 5, the party is good
    return 1;
}
    ...
    // Check the temperature limits based on whether it's summer or not
    if (isSummer) {
    ...
    if (isBirthday) {
        speed -= 5; // Increase the tolerance by 5
    }
    ...
    // Check if either you or your date is very stylish
    if (you >= 8 || date >= 8) {
        return 2; // Yes, you can get a table
    }
    // Check if either you or your date is not stylish at all
    else if (you <= 2 || date <= 2) {
        return 0; // No, you cannot get a table
    }
    // If neither condition is met, return maybe
    else {
        return 1; // Maybe, you might get a table
    }
}
    ...
    if (n % 3 == 0 && n % 5 == 0) {
        result = "FizzBuzz"; // Divisible by both
    } else if (n % 3 == 0) {
        result = "Fizz"; // Divisible by 3
    } else if (n % 5 == 0) {
        result = "Buzz"; // Divisible by 5
    } else {
        result = Integer.toString(n); // Not divisible by 3 or 5
    }

    // Return the result followed by "!"
    return result + "!";
}
    ...
    if (isWeekend) {
        // If it's the weekend, there is no upper limit, so we only need to check if it's at least 40
        return sodas >= 40;
    } else {
        // If it's not the weekend, check if the number of sodas is between 40 and 60 inclusive
